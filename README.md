...
**로컬에서만 쓸 거라면 서버 없이도 가능** — 두 방법 중 하나:

- `npx wrangler dev` — `workers/.dev.vars`에 `OPENAI_API_KEY=sk-...` 한 줄을 적고
  실행하면 `http://localhost:8787`이 로컬 엔드포인트가 됩니다.
- `js/config.local.example.js`를 `js/config.local.js`로 복사해 키 입력 — 페이지가
  API를 직접 호출합니다. 이 파일은 gitignore 처리되어 있지만, **공개 배포에는
  절대 사용하지 마세요** (키가 페이지 소스에 노출됩니다).

실제값 모드에서 무엇이 진짜인지:

| 실제 값 | 여전히 개념도 |
| --- | --- |
| 질문 토큰 분해·ID (js-tiktoken, 실제 o200k) | 어텐션 관계선 (API가 가중치를 제공하지 않음) |
| 답변 텍스트 · 답변 토큰 단위 | 벡터 히트맵 숫자 · 의미공간 좌표 |
| 단계별 top-5 확률 · 실제 선택된 토큰 | FFN·층 하강 연출 |

참고: `바다는`이 `바+다는`처럼 직관에 어긋나게 잘리는 것은 버그가 아니라 실제
BPE의 동작입니다 — 분할 기준이 언어학이 아니라 통계이기 때문입니다(씬 2 심화 참조).
...
**ローカルだけで使うならサーバー不要** ― どちらかの方法で：

- `npx wrangler dev` ― `workers/.dev.vars` に `OPENAI_API_KEY=sk-...` と書いて実行すると
  `http://localhost:8787` がローカルのエンドポイントになります。
- `js/config.local.example.js` を `js/config.local.js` にコピーしてキーを記入 ―
  ページが API を直接呼びます。このファイルは gitignore 済みですが、
  **公開デプロイには絶対に使わないでください**（キーがページソースに露出します）。

実測値モードで何が本物か：

| 実際の値 | 引き続き概念図 |
| --- | --- |
| 質問のトークン分割・ID（js-tiktoken、実際の o200k） | アテンションの関係線（API は重みを提供しない） |
| 回答テキスト · 回答のトークン単位 | ベクトルのヒートマップ数値 · 意味空間の座標 |
| ステップごとの top-5 確率 · 実際に選ばれたトークン | FFN・層降下の演出 |

補足：「바다는」が「바+다는」のように直感に反して切れるのはバグではなく、実際の
BPE の挙動です ― 分割基準が言語学ではなく統計だからです（シーン2の DEEP DIVE 参照）。
...
**Local-only? No server needed** — either of:

- `npx wrangler dev` — put `OPENAI_API_KEY=sk-...` in `workers/.dev.vars` and run;
  `http://localhost:8787` becomes your local endpoint.
- Copy `js/config.local.example.js` to `js/config.local.js` and add your key —
  the page calls the API directly. The file is git-ignored, but **never use this
  for a public deployment** (the key would be visible in the page source).

What is real in real-values mode:

| Real | Still conceptual |
| --- | --- |
| Question token splits & IDs (js-tiktoken, real o200k) | Attention wires (APIs don't expose weights) |
| Answer text · answer token boundaries | Vector heatmap numbers · meaning-space coords |
| Per-step top-5 probabilities · the actually-sampled token | FFN / layer-descent choreography |

Note: Korean "바다는" splitting as 바+다는 is not a bug — it's genuine BPE behaviour,
because the criterion is statistics, not linguistics (see the Scene 2 deep dive).
